"""
Test 1.5B LoRA checkpoints on Modal.
"""

import modal

app = modal.App("countdown-test-1.5b")
volume = modal.Volume.from_name("training-vol")

image = (
    modal.Image.debian_slim(python_version="3.11")
    .pip_install("torch", "transformers", "peft")
)


@app.function(
    image=image,
    gpu="A100",
    timeout=1800,
    volumes={"/output": volume},
)
def test():
    import re
    import torch
    from transformers import AutoTokenizer, AutoModelForCausalLM
    from peft import PeftModel

    MODEL_NAME = "Qwen/Qwen2.5-3B-Instruct"

    TEST_CASES = [
        {"target": 10, "nums": [2, 3, 5]},
        {"target": 24, "nums": [1, 2, 3, 4]},
        {"target": 15, "nums": [7, 8, 1]},
        {"target": 100, "nums": [25, 50, 75, 3]},
        {"target": 42, "nums": [10, 20, 6, 8]},
        {"target": 5, "nums": [2, 3]},
        {"target": 12, "nums": [3, 4, 6]},
        {"target": 30, "nums": [5, 6, 10]},
    ]

    def make_prompt(target, nums):
        return (
            "Using the numbers " + str(nums) + ", create an equation that equals " + str(target) + ". "
            "You can use basic arithmetic operations (+, -, *, /) and each number can only be used once. "
            "Show your work in <think> </think> tags. "
            "And return the final answer in <answer> </answer> tags, "
            "for example <answer> (1 + 2) / 3 </answer>."
        )

    def extract_answer(text):
        match = re.search(r"<answer>(.*?)</answer>", text, re.DOTALL)
        if not match:
            return None
        equation = match.group(1).strip()
        parts = equation.split("=")
        if len(parts) > 1:
            equation = parts[0].strip()
        return equation

    def validate(equation, target, numbers):
        try:
            used = [int(n) for n in re.findall(r"\d+", equation)]
            avail = list(numbers)
            for n in used:
                if n in avail:
                    avail.remove(n)
                else:
                    return False, "used invalid number: " + str(n)
            allowed = re.compile(r"^[\d+\-*/().\s]+$")
            if not allowed.match(equation):
                return False, "invalid characters"
            result = eval(equation)
            if abs(result - target) < 1e-6:
                return True, "= " + str(result)
            return False, "= " + str(result) + " (expected " + str(target) + ")"
        except Exception as e:
            return False, str(e)

    def generate(model, tokenizer, prompt):
        messages = [{"role": "user", "content": prompt}]
        text = tokenizer.apply_chat_template(messages, tokenize=False, add_generation_prompt=True)
        inputs = tokenizer(text, return_tensors="pt").to(model.device)
        with torch.no_grad():
            output = model.generate(
                **inputs, max_new_tokens=256, temperature=0.7, do_sample=True, top_p=0.9,
            )
        return tokenizer.decode(output[0][inputs.input_ids.shape[1]:], skip_special_tokens=True)

    def test_model(model, tokenizer, label):
        print("")
        print("=" * 60)
        print("  " + label)
        print("=" * 60)
        correct = 0
        for tc in TEST_CASES:
            prompt = make_prompt(tc["target"], tc["nums"])
            response = generate(model, tokenizer, prompt)
            print("")
            print("Target: " + str(tc["target"]) + ", Numbers: " + str(tc["nums"]))
            print("Response: " + response[:300])
            equation = extract_answer(response)
            if equation:
                valid, detail = validate(equation, tc["target"], tc["nums"])
                status = "CORRECT" if valid else "WRONG"
                if valid:
                    correct += 1
                print("Equation: " + equation + " -> " + status + " (" + detail + ")")
            else:
                print("No <answer> tags found")
        print("")
        print("Score: " + str(correct) + "/" + str(len(TEST_CASES)))
        return correct

    tokenizer = AutoTokenizer.from_pretrained(MODEL_NAME)

    # Test base model
    print("Loading base model...")
    base_model = AutoModelForCausalLM.from_pretrained(
        MODEL_NAME, dtype=torch.bfloat16, device_map="auto"
    )
    test_model(base_model, tokenizer, "Base Qwen2.5-3B-Instruct")
    del base_model
    torch.cuda.empty_cache()

    # Test checkpoints
    for ckpt in ["checkpoint-100", "checkpoint-200", "checkpoint-300", "checkpoint-400"]:
        path = "/output/countdown-3b-500/" + ckpt
        label = "LoRA 3B " + ckpt

        print("")
        print("Loading " + label + "...")
        base_model = AutoModelForCausalLM.from_pretrained(
            MODEL_NAME, dtype=torch.bfloat16, device_map="auto"
        )
        lora_model = PeftModel.from_pretrained(base_model, path)
        lora_model.eval()
        test_model(lora_model, tokenizer, label)
        del lora_model, base_model
        torch.cuda.empty_cache()


@app.local_entrypoint()
def main():
    test.remote()