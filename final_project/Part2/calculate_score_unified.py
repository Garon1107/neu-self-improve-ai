...
The prediction is correct if the Model Response contains an answer that is semantically equivalent to the Correct Answer. Follow these instructions carefully:

1. Extract the core predicted answer from the Model Response. Ignore explanations, process summaries, and formatting.
2. Compare the extracted answer with the Correct Answer. Consider them matching if they refer to the same entity, value, or concept, even if worded differently.
3. Minor differences in formatting, capitalization, or extra details (e.g., "James Madison" vs "james madison", "1999" vs "in 1999") should still be considered correct.
...
Response Format:
<analysis>: First extract the core answer from the model response, then explain the comparison with the correct answer
<true_false>: Return "True" if the answers match semantically, otherwise "False"
        """
